...
def _query(conn, permission: DB_Permission, table: str, view: View,
           order_by: str = "", limit: int = -1, offset: int = 0) -> list[dict]:
    return ops.query(conn, permission, table, view, order_by, limit, offset)


def _add(conn, permission: DB_Permission, notetype: str, entries: list[dict]) -> list[dict]:
    if notetype not in registry:
        raise ToolError(f"不支持的笔记类型: {notetype}")
    results = ops.add(conn, permission, notetype, entries)
    ids = [r["id"] for r in results if "id" in r]
    return ops.update(conn, root_permission(conn.db), notetype,
                      Condition("id", ids, "IN"), {"is_ai_gen": 1})


def _update(conn, permission: DB_Permission, notetype: str, filter, values: dict) -> list[dict]:
    return ops.update(conn, permission, notetype, filter, values)


def _delete(conn, permission: DB_Permission, notetype: str, filter) -> list[dict]:
    return ops.delete(conn, permission, notetype, filter)
...
_ToolFactory = Callable[[DB_Permission], BaseTool]


def _make_query_entries(permission: DB_Permission) -> BaseTool:
    @tool
    def query_entries(
        view: ViewModel,
        notetype: str,
        ...
        """
        return _with_read_tool(lambda conn: _query(conn, permission, notetype, view.to_view(), order_by, limit, offset))
    return query_entries


def _make_add_entries(permission: DB_Permission) -> BaseTool:
    @tool
    def add_entries(notetype: str, entries: list[dict[str, Any]]) -> dict:
        ...
        """
        return _with_write_tool(lambda conn: _add(conn, permission, notetype, entries))
    return add_entries


def _make_update_entries(permission: DB_Permission) -> BaseTool:
    @tool
    def update_entries(notetype: str, filter: FilterModel, values: dict[str, Any]) -> dict:
        ...
        """
        return _with_write_tool(lambda conn: _update(conn, permission, notetype, filter.to_filter(), values))
    return update_entries


def _make_delete_entries(permission: DB_Permission) -> BaseTool:
    @tool
    def delete_entries(notetype: str, filter: FilterModel) -> dict:
        ...
        """
        return _with_write_tool(lambda conn: _delete(conn, permission, notetype, filter.to_filter()))
    return delete_entries


def _make_get_ai_permission(permission: DB_Permission) -> BaseTool:
    @tool
    def get_ai_permission() -> dict:
        ...
        Returns:
            ...
            格式：{"read": {...}, "write": {...}}
        """
        read_view, write_view = permission
        return {
            "read": view_to_json(read_view),
            "write": view_to_json(write_view),
        }
    return get_ai_permission


_TOOL_REGISTRY: dict[str, _ToolFactory] = {
    "query_entries":     _make_query_entries,
    "add_entries":       _make_add_entries,
    "update_entries":    _make_update_entries,
    "delete_entries":    _make_delete_entries,
    "get_ai_permission": _make_get_ai_permission,
}

DEFAULT_TOOL_NAMES = list(_TOOL_REGISTRY.keys())


def make_tools(tool_names: list[str], permission: DB_Permission) -> list[BaseTool]:
    """按名称列表和权限构建工具列表。

    Args:
        tool_names: 工具名称列表，如 ["query_entries", "add_entries"]。
        permission: 要绑定到所有工具上的权限。
