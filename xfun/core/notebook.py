...
class Notebook:
    ...
    def _validate(self, entry: dict[str, Any]) -> None:
        """校验钩子：本子特有的校验逻辑。"""
        if "is_ai_gen" in entry and entry["is_ai_gen"] is not None:
            if entry["is_ai_gen"] not in (0, 1):
                raise EntryInvalidError("notebook", f"is_ai_gen 必须为 0 或 1，实际: {entry['is_ai_gen']}")
