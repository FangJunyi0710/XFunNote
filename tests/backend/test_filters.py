...
class TestGetFilter:
    ...
    def test_get(self, client, demo_filter):
        resp = client.get(f"/api/v1/filters/{demo_filter['name']}")
        assert resp.status_code == 200
        data = resp.json()
        assert isinstance(data, dict)

    def test_get_not_found(self, client):
        resp = client.get("/api/v1/filters/nonexistent")
...
class TestSaveFilter:
    ...
    def test_update(self, client, demo_filter):
        resp = client.put(
            f"/api/v1/filters/{demo_filter['name']}",
...
class TestDeleteFilter:
    ...
    def test_delete(self, client, demo_filter):
        resp = client.delete(f"/api/v1/filters/{demo_filter['name']}")
        assert resp.status_code == 200
        assert "已删除" in resp.json()["message"]

    def test_delete_not_found(self, client):
        resp = client.delete("/api/v1/filters/nonexistent")
        assert resp.status_code == 404
