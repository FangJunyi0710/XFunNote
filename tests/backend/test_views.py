...
class TestGetView:
    ...
    def test_get(self, client, demo_view):
        resp = client.get(f"/api/v1/views/{demo_view['name']}")
        assert resp.status_code == 200
        data = resp.json()
...
class TestSaveView:
    ...
    def test_update(self, client, demo_view):
        resp = client.put(
            f"/api/v1/views/{demo_view['name']}",
            json={"plan": [{"columns": ["content", "month"], "filter": []}]},
        )
        assert resp.status_code == 200
        # 验证已更新
        get_resp = client.get(f"/api/v1/views/{demo_view['name']}")
...
class TestDeleteView:
    ...
    def test_delete(self, client, demo_view):
        resp = client.delete(f"/api/v1/views/{demo_view['name']}")
        assert resp.status_code == 200
