...
| **AI Tools 层** | 8 个 Function Calling 工具（`query_entries`、`add_entries`、`update_entries`、`delete_entries`、`manage_tags`、`add_ai_note`、`search_memories`、`save_memory`）+ 行级/列级安全沙箱（`AI_READ_FILTER`、`AI_WRITE_FILTER`、`AI_WRITABLE_COLUMNS`）+ LangChain Agent 对话 + 系统提示词 |
| **视图层** | `xfun/core/view.py` 实现跨本子数据水合查询，支持 `fields` 限定列、自动合并 AI 安全沙箱 |
| **测试覆盖** | 全面覆盖核心引擎正常路径、边界条件、错误路径及事务回滚 94+ 个单元测试 |
...
### 3.1 记忆系统的持续演化（远期）
- **导入器机制**：将外部文本（AI 对话、日记、笔记）转换为 `accumulation` 条目，作为原始记忆素材。
- **自动学习器**：定期扫描新导入的条目，调用 AI 生成 `ai_tags`，并将相关内容提炼为结构化记忆。
- **聊天即记忆**：每次与 AI 的对话结束后，自动保存关键结论到 `accumulation`，实现"对话即积累"。
...
| 优先级 | 阶段 | 产出 |
| :--- | :--- | :--- |
| ✅ 已完成 | AI Tools 层 | `xfun/ai/security.py` + `xfun/ai/tools.py`（8 个工具）+ `xfun/ai/agent.py` + `xfun/ai/prompts.py` |
