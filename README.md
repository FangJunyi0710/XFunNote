...
AI 的"懂你"能力来源于系统内沉淀的多维度数据（plan/timeline/diary/word/accumulation/aimemory），当这些数据在同一个系统里积累足够长时间后，AI 能自然地感知时间跨度和感情变化，产生个性化的反馈。
...
- **多维数据感知**：AI 的"懂你"源于 plan（意图）、timeline（行为）、diary（感受）、word（输入）等数据在同一系统中长期沉淀，使其能自然感知时间跨度与情感变化。
...
- **身份即视角**：以 `(read_view, write_view)` 元组定义"身份"，内置 root/ai/no 三种预设，用户可自由组合创建新身份。
...
- **系统支撑表**：`_token`（鉴权）、`_permission`（身份）、`_view`（视图）、`_filter`（筛选），通过对应路由管理。
...
|------|---------|------|
| `/` | Home | 首页 |
| `/notebooks/:type` | Notebook*Page（7 个） | plan/diary/word/accumulation/aimemory/timeline/schedule |
...
- [x] 7 个内置本子（plan/diary/word/accumulation/aimemory/timeline/schedule）
- [x] 注册中心
- [x] 300+ 单元测试
...
**鉴权说明**：除 Shortcut 兑换外，所有路由均需 `X-API-Key` Header。`/db/*` 路由必须使用 `ROOT_TOKEN`。每次请求自动计算 API Key 权限与业务权限的交集。
...
**数据库在线热备份**：基于 SQLite `backup()` API，无需停服。备份文件存储于 `data/backups/`，文件名带时间戳。
...
22 个测试文件覆盖：核心引擎（db/filter/notebook/ops/view/extras）、全部 7 个内置本子、AI 层（agent/prompts/schema/tools）、工具函数、注册中心。
